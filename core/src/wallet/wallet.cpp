...
/**
 * Fees smaller than this (in umubdi) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then mubdi now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to mubdi.
...
PairResult CWallet::getNewAddress(CMuBdIAddress& ret, std::string label){
    return getNewAddress(ret, label, AddressBook::AddressBookPurpose::RECEIVE);
}

PairResult CWallet::getNewStakingAddress(CMuBdIAddress& ret, std::string label){
    return getNewAddress(ret, label, AddressBook::AddressBookPurpose::COLD_STAKING, CChainParams::Base58Type::STAKING_ADDRESS);
}

PairResult CWallet::getNewAddress(CMuBdIAddress& ret, const std::string addressLabel, const std::string purpose,
                                         const CChainParams::Base58Type addrType)
{
    ...
    ret = CMuBdIAddress(keyID, addrType);
...
int64_t CWallet::GetKeyCreationTime(const CMuBdIAddress& address)
{
    CKeyID keyID;
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CMuBdIAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(&vPossibleCoins, true, NULL, false, ONLY_5000, false, 1, false, false);
...
bool CWallet::GetVinAndKeysFromOutput(COutput out, CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, bool fColdStake)
{
    ...
    CMuBdIAddress address2(address1);
...
bool CWallet::IsUsed(const CMuBdIAddress address) const
{
    LOCK(cs_wallet);
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == STAKEABLE_COINS) continue;
...
std::map<CMuBdIAddress, std::vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    std::vector<COutput> vCoins;
    // include cold and delegated coins
    AvailableCoins(&vCoins, fConfirmed, nullptr, false, ALL_COINS, false, 1, true, true);

    std::map<CMuBdIAddress, std::vector<COutput> > mapCoins;
    for (COutput out : vCoins) {
        ...
        mapCoins[CMuBdIAddress(address, fColdStakeAddr ? CChainParams::STAKING_ADDRESS : CChainParams::PUBKEY_ADDRESS)].push_back(out);
    }
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 CAI collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 MuBdI collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 5000 CArIa.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT5000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 5000 CArIa.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-mubdi-address
                    CScript scriptChange;
        ...
        )
{
    ...
    // Parse utxos into CMuBdIStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CMuBdIStake> input(new CMuBdIStake());
...
CMuBdIAddress CWallet::ParseIntoAddress(const CTxDestination& dest, const std::string& purpose) {
    const CChainParams::Base58Type addrType =
            AddressBook::IsColdStakingPurpose(purpose) ?
            CChainParams::STAKING_ADDRESS : CChainParams::PUBKEY_ADDRESS;
    return CMuBdIAddress(dest, addrType);
}
...
{
    std::string strAddress =  CMuBdIAddress(address, addrType).ToString();
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CMuBdIAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CMuBdIAddress(dest).ToString(), key);
}
...
void CWallet::AutoCombineDust()
{
    ...
    std::map<CMuBdIAddress, std::vector<COutput> > mapCoinsByAddress = AvailableCoinsByAddress(true, nAutoCombineThreshold * COIN);

    //coins are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (std::map<CMuBdIAddress, std::vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
    ...
    for (const COutput& out : vCoins) {
        ...
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CMuBdIAddress(destMyAddress).ToString()) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            CMuBdIAddress strAddSend(vMultiSend[i].first);
