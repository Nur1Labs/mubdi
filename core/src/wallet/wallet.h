...
#ifndef MUBDI_WALLET_H
#define MUBDI_WALLET_H
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4,           // ONLY_NONDENOMINATED and not 5000 MuBdI at the same time
...
// Possible states for zMuBdI send
enum ZerocoinSpendStatus {
    ZMUBDI_SPEND_OKAY = 0,                            // No error
    ZMUBDI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMUBDI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMUBDI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMUBDI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMUBDI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMUBDI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMUBDI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMUBDI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMUBDI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMUBDI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMUBDI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMUBDI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMUBDI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMUBDI_SPENT_USED_ZMUBDI = 14,              // Coin has already been spend
    ZMUBDI_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    std::map<CMuBdIAddress, std::vector<COutput> > AvailableCoinsByAddress(bool fConfirmed = true, CAmount maxCoinValue = 0);
    bool SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet) const;

    /// Get 5000 MuBdI output and keys which can be used for the Masternode
    ...
    PairResult getNewAddress(CMuBdIAddress& ret, const std::string addressLabel, const std::string purpose,
                                           const CChainParams::Base58Type addrType = CChainParams::PUBKEY_ADDRESS);
    PairResult getNewAddress(CMuBdIAddress& ret, std::string label);
    PairResult getNewStakingAddress(CMuBdIAddress& ret, std::string label);
    int64_t GetKeyCreationTime(CPubKey pubkey);
    int64_t GetKeyCreationTime(const CMuBdIAddress& address);
    ...
    bool IsUsed(const CMuBdIAddress address) const;
    ...
    static CMuBdIAddress ParseIntoAddress(const CTxDestination& dest, const std::string& purpose);
...
public:
    ...
    int Priority() const
    {
        for (CAmount d : obfuScationDenominations)
            if (tx->vout[i].nValue == d) return 5000;
