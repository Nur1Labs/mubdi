...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name") {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CMuBdIAddress(strAddress).Get()].name;
        } else if (strType == "purpose") {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CMuBdIAddress(strAddress).Get()].purpose;
        } else if (strType == "tx") {
            uint256 hash;
            ssKey >> hash;
            CWalletTx wtx;
        ...
        } else if (strType == "multisend") //presstab HyperStake
        {
            ...
            std::pair<std::string, int> pMultiSend;
            ssValue >> pMultiSend;
            if (CMuBdIAddress(pMultiSend.first).IsValid()) {
        ...
        } else if (strType == "mdisabled") //presstab HyperStake
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CMuBdIAddress(strAddress).Get(), strKey, strValue)) {
...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("mubdi-wallet");
