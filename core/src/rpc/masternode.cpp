...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) CArIa address of current masternode\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode CArIa address\n"
    ...
    for (PAIRTYPE(int, CMasternode) & s : vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == std::string::npos &&
                ...
                CMuBdIAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == std::string::npos) continue;
            ...
            obj.push_back(Pair("addr", CMuBdIAddress(mn->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:46031\"") + HelpExampleRpc("masternodeconnect", "\"192.168.1.3:45088\""));
...
UniValue masternodecurrent (const UniValue& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", CMuBdIAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue createmasternodekey (const UniValue& params, bool fHelp)
{
    ...
    return CMuBdISecret(secret).ToString();
}
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) CArIa address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
    ...
    if (pmn) {
        ...
        mnObj.push_back(Pair("addr", CMuBdIAddress(pmn->pubKeyCollateralAddress.GetID()).ToString()));
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) CArIa MN Address\n"
...
UniValue decodemasternodebroadcast(const UniValue& params, bool fHelp)
{
    ...
    resultObj.push_back(Pair("pubkeycollateral", CMuBdIAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
    resultObj.push_back(Pair("pubkeymasternode", CMuBdIAddress(mnb.pubKeyMasternode.GetID()).ToString()));
