...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total caria balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in caria/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in caria/kb\n"
...
public:
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        for (const CTxDestination& addr : addresses)
            a.push_back(CMuBdIAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"mubdiaddress\"\n"
            "\nReturn information about the given caria address.\n"

            "\nArguments:\n"
            "1. \"mubdiaddress\"     (string, required) The caria address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mubdiaddress\",    (string) The caria address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for CArIa cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MuBdI address and we have full public key:
        CMuBdIAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are caria addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) caria address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"mubdiaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"mubdiaddress\"  (string, required) The caria address to use for the signature.\n"
