...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 MuBdI finalization collateral
            //       will also cover the old 50 MuBdI finalization collateral.
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight <= last_pow_block && nHeight >= 127340) {
        nSubsidy = 50 * COIN;
    ...
    // Amount of blocks in a months period of time (using specific number 5772*10) = (57720)
    // was - amount of block in a months period of time (using 1 minute per) = (60*24*30)
    if (nHeight <= 172800) {                                        //height get from a month * by quarterly = (60*24*30*4)
        return 601000 * COIN;
    } else {
