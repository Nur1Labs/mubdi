...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zMuBdI spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("MuBdIMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("MuBdIMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MuBdIMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("mubdi-miner");
    ...
    while (fGenerateMuBdIs || fProofOfStake) {
        CBlockIndex* pindexPrev = GetChainTip();
        if (!pindexPrev) {
        ...
        if (fProofOfStake) {
            if (pindexPrev->nHeight < start_pos_block) {
        ...
        LogPrintf("Running MuBdIMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadMuBdIMiner(void* parg)
{
    boost::this_thread::interruption_point();
    CWallet* pwallet = (CWallet*)parg;
    try {
        MuBdIMiner(pwallet, false);
        boost::this_thread::interruption_point();
    } catch (const std::exception& e) {
        LogPrintf("MuBdIMiner exception");
    } catch (...) {
        LogPrintf("MuBdIMiner exception");
    }

    LogPrintf("MuBdIMiner exiting\n");
}

void GenerateMuBdIs(bool fGenerate, CWallet* pwallet, int nThreads)
{
    static boost::thread_group* minerThreads = NULL;
    fGenerateMuBdIs = fGenerate;
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadMuBdIMiner, pwallet));
}
