...
CZerocoinDB::CZerocoinDB(size_t nCacheSize, bool fMemory, bool fWipe) : CLevelDBWrapper(GetDataDir() / "airlogic", nCacheSize, fMemory, fWipe)
{
}
...
bool CZerocoinDB::WriteCoinMint(const libzerocoin::PublicCoin& pubCoin, const uint256& hashTx)
{
    ...
    return Write(std::make_pair('m', hash), hashTx, true);
}
...
bool CZerocoinDB::WriteCoinSpend(const CBigNum& bnSerial, const uint256& txHash)
{
    ...
    return Write(std::make_pair('s', hash), txHash, true);
}
