...
{
    CMuBdIAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered mubdi address / amount:
            ...
            CMuBdIAddress out = CMuBdIAddress(rcp.address.toStdString());

            if (rcp.isP2CS) {
                CMuBdIAddress ownerAdd;
                if (rcp.ownerAddress.isEmpty()) {
                    ...
                    ownerAdd = CMuBdIAddress(rcp.ownerAddress.toStdString());
                }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal mubdi:URI (mubdi:XyZ...?message=example)
            {
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, transaction.getRecipients()) {
        ...
        if (!rcp.paymentRequest.IsInitialized()) {
            CMuBdIAddress address = CMuBdIAddress(rcp.address.toStdString());
...
int64_t WalletModel::getKeyCreationTime(const CMuBdIAddress& address){
...
PairResult WalletModel::getNewAddress(CMuBdIAddress& ret, std::string label) const{
    return wallet->getNewAddress(ret, label);
}

PairResult WalletModel::getNewStakingAddress(CMuBdIAddress& ret,std::string label) const{
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    CMuBdIAddress address(addressStr.toStdString());
    if (address.IsStakingAddress())
        return error("Invalid CArIa address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CMuBdIAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid CArIa address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from CArIa address");

    return true;
}

std::string WalletModel::getLabelForAddress(const CMuBdIAddress& address) {
    std::string label = "";
    {
        LOCK(wallet->cs_wallet);
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CMuBdIAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CMuBdIAddress(sAddress).Get();
...
bool WalletModel::isMine(CMuBdIAddress address)
{
...
bool WalletModel::isUsed(CMuBdIAddress address)
{
