                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select CAI Denominations to Spend");
    ...
    // see https://github.com/mubdi/mubdi/issues/5716
...
{
    GUIUtil::setClipboard(MuBdIUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
{
    GUIUtil::setClipboard(MuBdIUnits::removeSpaces(ui->labelCoinControlAmount->text().left(ui->labelCoinControlAmount->text().indexOf(" "))));
...
{
    GUIUtil::setClipboard(MuBdIUnits::removeSpaces(ui->labelCoinControlFee->text().left(ui->labelCoinControlFee->text().indexOf(" ")).replace("~", "")));
...
{
    GUIUtil::setClipboard(MuBdIUnits::removeSpaces(ui->labelCoinControlAfterFee->text().left(ui->labelCoinControlAfterFee->text().indexOf(" ")).replace("~", "")));
...
{
    GUIUtil::setClipboard(MuBdIUnits::removeSpaces(ui->labelCoinControlChange->text().left(ui->labelCoinControlChange->text().indexOf(" ")).replace("~", "")));
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = MuBdIUnits::MuBdI;
    ...
    l2->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, nAmount));   // Amount
    l3->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, nPayFee));   // Fee
    l4->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));   // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                          // Dust
    l8->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, nChange));   // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(MuBdIUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(MuBdIUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(MuBdIUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
    ...
    QString toolTip4 = tr("Can vary +/- %1 ucai per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                sAddress = QString::fromStdString(CMuBdIAddress(outputAddress).ToString());

                // if listMode or change => show MuBdI address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, nSum));
