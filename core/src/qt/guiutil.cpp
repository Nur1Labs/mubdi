...
#include "mubdiaddressvalidator.h"
...
{
    CAmount val = 0;
    bool valid = MuBdIUnits::parse(displayUnit, text, &val);
    if (valid) {
        if (val < 0 || val > MuBdIUnits::maxMoney())
            valid = false;
    }
    if (valid_out)
        *valid_out = valid;
...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZMuBdI){
    return (amount == 0) ? ("0.00 " + MuBdIUnits::name(nDisplayUnit, isZMuBdI)) : MuBdIUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, MuBdIUnits::separatorAlways, true, isZMuBdI);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(MuBdIAddressFont());
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter CArIa address (e.g. %1)").arg("caiFR83SQbiezrW72hjcWJtcfip5krte2Z"));
    widget->setValidator(new MuBdIAddressEntryValidator(parent));
    widget->setCheckValidator(new MuBdIAddressCheckValidator(parent));
}
...
bool parseMuBdIURI(const QUrl& uri, SendCoinsRecipient* out)
{
    // return if URI is not valid or is no MuBdI: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!MuBdIUnits::parse(MuBdIUnits::MuBdI, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseMuBdIURI(QString uri, SendCoinsRecipient* out)
{
    // Convert mubdi:// to mubdi:
    //
    //    Cannot handle this later, because mubdi:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith(URI_SCHEME "://", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SCHEME) + 3, URI_SCHEME ":");
    }
    QUrl uriInstance(uri);
    return parseMuBdIURI(uriInstance, out);
}

QString formatMuBdIURI(const SendCoinsRecipient& info)
{
    QString ret = QString(URI_SCHEME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(MuBdIUnits::format(MuBdIUnits::MuBdI, info.amount, false, MuBdIUnits::separatorNever));
        paramCount++;
    }
...
{
    CTxDestination dest = CMuBdIAddress(address.toStdString()).Get();
...
bool openConfigfile()
{
    ...
    /* Open mubdi.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CArIa.lnk";
}
...
{
    return GetAutostartDir() / "CArIa.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mubdi.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CArIa\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mubdi app
...
bool GetStartOnSystemStartup()
{
    CFURLRef MuBdIAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, MuBdIAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mubdi app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, MuBdIAppUrl, NULL, NULL);
