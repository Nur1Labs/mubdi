...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", MuBdIUnits::MuBdI);
    ...
    if (settings.contains("nAnonymizeMuBdIAmount") || reset)
        SoftSetArg("-anonymizemubdiamount", settings.value("nAnonymizeMuBdIAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mubdi.cpp during shotdown to also remove the window positions
