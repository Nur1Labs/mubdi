...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    bool fSpendFromMe = false;

    if (wtx.IsCoinStake()) {
        TransactionRecord sub(hash, nTime, wtx.GetTotalSize());

        CTxDestination address;
        if (!wtx.HasZerocoinSpendInputs() && !IsMine(*wallet, address)) {
            ...
            for (unsigned int i = 1; i < wtx.vout.size(); i++) {
                ...
                if (ExtractDestination(wtx.vout[i].scriptPubKey, outAddress)) {
                    if (IsMine(*wallet, outAddress)) {
                        ...
                        sub.address = CMuBdIAddress(outAddress).ToString();
            ...
            if (wtx.HasP2CSOutputs()) {
                ...
                sub.address = CMuBdIAddress(address).ToString();
                sub.credit = nNet;
            }
        } else {
            //Masternode reward
            ...
            if (ExtractDestination(wtx.vout[nIndexMN].scriptPubKey, destMN) && IsMine(*wallet, destMN)) {
                ...
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::MNReward;
                sub.address = CMuBdIAddress(destMN).ToString();
