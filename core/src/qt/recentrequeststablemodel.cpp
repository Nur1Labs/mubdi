    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return MuBdIUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, MuBdIUnits::separatorNever);
            else
                return MuBdIUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    if (this->walletModel->getOptionsModel() != NULL) {
        amountTitle += " (" + MuBdIUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
