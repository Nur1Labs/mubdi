...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    if (wtx.IsCoinBase()) {
        ...
        if (nNet > 0) {
            // Credit
            if (CMuBdIAddress(rec->address).IsValid()) {
                CTxDestination address = CMuBdIAddress(rec->address).Get();
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty()) {
        ...
        strHTML += "<b>" + tr("To") + ":</b> ";
        CTxDestination dest = CMuBdIAddress(strAddress).Get();
    ...
    if (wtx.IsCoinBase() && rec->credit == 0) {
        ...
        if (wtx.IsInMainChain())
            strHTML += MuBdIUnits::formatHtmlWithUnit(unit, nUnmatured) + " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut& txout : wtx.vout) {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty()) {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address)) {
                        ...
                        strHTML += GUIUtil::HtmlEscape(CMuBdIAddress(address).ToString());
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if (toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe) {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = rec->debit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        } else {
            //
            ...
            for (const CTxIn& txin : wtx.vin)
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, -wallet->GetDebit(txin, ISMINE_ALL)) + "<br>";
            for (const CTxOut& txout : wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + MuBdIUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal mubdi:URI (mubdi:XyZ...?message=example)
            ...
            if (pcoinsTip->GetCoins(prevout.hash, prev)) {
                if (prevout.n < prev.vout.size()) {
                    ...
                    if (ExtractDestination(vout.scriptPubKey, address)) {
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].name.empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address].name) + " ";
                        strHTML += QString::fromStdString(CMuBdIAddress(address).ToString());
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + MuBdIUnits::formatHtmlWithUnit(unit, vout.nValue);
