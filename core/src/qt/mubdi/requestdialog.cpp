...
#include "qt/mubdi/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a CArIa address, you can create a payment request, bundling up more information."));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CAI");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + MuBdIUnits::format(displayUnit, value, false, MuBdIUnits::separatorAlways) + " CAI";
        } else {
...
void RequestDialog::onCopyUriClicked(){
    if(info) {
        GUIUtil::setClipboard(GUIUtil::formatMuBdIURI(*info));
        res = 1;
        accept();
    }
}
