...
#include "qt/mubdi/addresseswidget.h"
#include "qt/mubdi/forms/ui_addresseswidget.h"
#include "qt/mubdi/addresslabelrow.h"
#include "qt/mubdi/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(MuBdIGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter CArIa address"));
    setCssProperty(ui->labelAddress, "text-title");
    ui->lineEditAddress->setPlaceholderText("e.g. caiFR83SQbiezrW72hjc…");
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CMuBdIAddress MuBdIAdd = CMuBdIAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(MuBdIAdd.Get(), label.toUtf8().constData(),
                MuBdIAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
...
void AddressesWidget::onEditClicked(){
    ...
    if(openDialogWithOpaqueBackground(dialog, window)){
        if(walletModel->updateAddressBookLabels(
                CMuBdIAddress(address.toStdString()).Get(), dialog->getLabel().toStdString(), addressTablemodel->purposeForAddress(address.toStdString()))){
