...
#include "qt/mubdi/dashboardwidget.h"
#include "qt/mubdi/forms/ui_dashboardwidget.h"
...
DashboardWidget::DashboardWidget(MuBdIGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of CAI and XAU staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareMuBdI, "square-chart-mubdi");
    setCssProperty(ui->labelSquarezMuBdI, "square-chart-zmubdi");
    setCssProperty(ui->labelMuBdI, "text-chart-mubdi");
    setCssProperty(ui->labelZMuBdI, "text-chart-zmubdi");
    ...
    ui->labelAmountZMuBdI->setText("0 XAU");
    ui->labelAmountMuBdI->setText("0 CAI");
    setCssProperty(ui->labelAmountMuBdI, "text-stake-mubdi-disable");
    setCssProperty(ui->labelAmountZMuBdI, "text-stake-zmubdi-disable");
    ...
    ui->btnHowTo->setText(tr("How to get CAI or XAU"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMuBdI = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZMuBdI;
        ...
        if (amountBy.contains(time)) {
            ...
            if (isMuBdI) {
                ...
                hasZMuBdIStakes = false; //disable z deprecated
            } else {
                amountBy[time] = std::make_pair(0, amount);
                hasZMuBdIStakes = false;
            }
        }
    }
    return amountBy;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair MuBdI, zMuBdI
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            MuBdI = (pair.first != 0) ? pair.first / 100000000 : 0;
            zMuBdI = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMuBdI += pair.first;
            chartData->totalZMuBdI += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMuBdI.append(MuBdI);
        chartData->valueszMuBdI.append(zMuBdI);

        int max = std::max(MuBdI, zMuBdI);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("CAI");
    set1 = new QBarSet("XAU");
    ...
    set0->append(chartData->valuesMuBdI);
    set1->append(chartData->valueszMuBdI);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMuBdI > 0 || chartData->totalZMuBdI > 0) {
        setCssProperty(ui->labelAmountMuBdI, "text-stake-mubdi");
        setCssProperty(ui->labelAmountZMuBdI, "text-stake-zmubdi");
    } else {
        setCssProperty(ui->labelAmountMuBdI, "text-stake-mubdi-disable");
        setCssProperty(ui->labelAmountZMuBdI, "text-stake-zmubdi-disable");
    }
    forceUpdateStyle({ui->labelAmountMuBdI, ui->labelAmountZMuBdI});
    ui->labelAmountMuBdI->setText(GUIUtil::formatBalance(chartData->totalMuBdI, nDisplayUnit));
    ui->labelAmountZMuBdI->setText(GUIUtil::formatBalance(chartData->totalZMuBdI, nDisplayUnit, true));
