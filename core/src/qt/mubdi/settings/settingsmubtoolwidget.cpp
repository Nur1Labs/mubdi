...
#include "qt/mubdi/settings/forms/ui_settingsmubtoolwidget.h"
...
SettingsMubToolWidget::SettingsMubToolWidget(MuBdIGUI* _window, QWidget *parent) :
    PWidget(_window, parent),
    ui(new Ui::SettingsMubToolWidget)
{
    ui->setupUi(this);
    ...
    // Address
    ui->labelSubtitleAddress->setText(tr("CArIa address"));
    ...
    connect(ui->pushButtonEncrypt, &QPushButton::clicked, this, &SettingsMubToolWidget::onEncryptKeyButtonENCClicked);
    ...
    connect(ui->pushButtonClear, &QPushButton::clicked, this, &SettingsMubToolWidget::onClearAll);
    connect(ui->pushButtonDecryptClear, SIGNAL(clicked()), this, SLOT(onClearDecrypt()));
}

void SettingsMubToolWidget::setAddress_ENC(const QString& address){
    ui->addressIn_ENC->setText(address);
    ui->passphraseIn_ENC->setFocus();
}

void SettingsMubToolWidget::onEncryptSelected(bool isEncr) {
...
void SettingsMubToolWidget::onEncryptKeyButtonENCClicked()
{
    if (!walletModel)
        return;
    ...
    CMuBdIAddress addr(ui->addressIn_ENC->text().toStdString());
    if (!addr.IsValid()) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
...
void SettingsMubToolWidget::onClearAll(){
...
void SettingsMubToolWidget::onAddressesClicked(){
...
void SettingsMubToolWidget::resizeMenu(){
...
void SettingsMubToolWidget::onClearDecrypt(){
...
void SettingsMubToolWidget::onDecryptClicked(){
    ...
    CMuBdIAddress address(pubKey.GetID());
    ui->lineEditDecryptResult->setText(QString::fromStdString(address.ToString()));
}

void SettingsMubToolWidget::importAddressFromDecKey(){
    WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::BIP_38, true));
    if (!ctx.isValid()) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
    ...
    CMuBdIAddress address(ui->lineEditDecryptResult->text().toStdString());
    CPubKey pubkey = key.GetPubKey();

    if (!address.IsValid() || !key.IsValid() || CMuBdIAddress(pubkey.GetID()).ToString() != address.ToString()) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
...
void SettingsMubToolWidget::resizeEvent(QResizeEvent *event){
    resizeMenu();
