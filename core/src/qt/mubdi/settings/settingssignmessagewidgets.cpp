...
#include "qt/mubdi/settings/forms/ui_settingssignmessagewidgets.h"
...
SettingsSignMessageWidgets::SettingsSignMessageWidgets(MuBdIGUI* _window, QWidget *parent) :
...
void SettingsSignMessageWidgets::onSignMessageButtonSMClicked(){
    ...
    CMuBdIAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
void SettingsSignMessageWidgets::onVerifyMessage(){
    ...
    if (!(CMuBdIAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
