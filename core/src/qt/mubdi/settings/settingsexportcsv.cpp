...
#include "qt/mubdi/settings/settingsexportcsv.h"
#include "qt/mubdi/settings/forms/ui_settingsexportcsv.h"
#include <QFile>
...
SettingsExportCSV::SettingsExportCSV(MuBdIGUI* _window, QWidget *parent) :
...
void SettingsExportCSV::selectFileOutput(const bool& isTxExport)
{
    QString filename = GUIUtil::getSaveFileName(this,
                                        ...
                                        isTxExport ? tr("MUBDI_tx_csv_export(*.csv)") : tr("MUBDI_addresses_csv_export(*.csv)"),
                                        nullptr);
...
void SettingsExportCSV::exportTxes(const QString& filename)
{
    ...
    if (walletModel) {
        ...
        writer.addColumn(MuBdIUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
