...
#include "qt/mubdi/settings/forms/ui_settingsmultisendwidget.h"
#include "qt/mubdi/settings/settingsmultisenddialog.h"
    ...
    ui(new Ui::SettingsMultisendWidget)
{
    ...
    ui->labelSubtitle1->setText(tr("MultiSend allows you to automatically send up to 100% of your stake or masternode reward to a list of other CArIa addresses after it matures."));
...
void SettingsMultisendWidget::addMultiSend(QString address, int percentage, QString addressLabel){
    ...
    if (!CMuBdIAddress(strAddress).IsValid()) {
...
void SettingsMultisendWidget::activate(){
    ...
    } else if (CMuBdIAddress(pwalletMain->vMultiSend[0].first).IsValid()) {
        pwalletMain->fMultiSendStake = ui->checkBoxStake->isChecked();
        pwalletMain->fMultiSendMasternodeReward = ui->checkBoxRewards->isChecked();
