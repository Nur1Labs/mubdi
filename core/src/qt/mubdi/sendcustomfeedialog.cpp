...
#include "qt/mubdi/sendcustomfeedialog.h"
#include "qt/mubdi/forms/ui_sendcustomfeedialog.h"
    ...
    ui(new Ui::SendCustomFeeDialog)
{
    ...
    ui->lineEditCustomFee->setPlaceholderText("0.000001 CAI");
...
void SendCustomFeeDialog::onCustomChecked(){
    ...
    if(walletModel && ui->lineEditCustomFee->text().isEmpty()) {
        ...
        ui->lineEditCustomFee->setText(MuBdIUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()));
    }
}
...
void SendCustomFeeDialog::updateFee(){
    ...
    if (res) {
        ...
        if (feeRate <= CFeeRate(0)) { // not enough data => minfee
            feeRate = CWallet::minTxFee;
            ui->labelFee->setText(MuBdIUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(),
