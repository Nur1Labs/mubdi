...
#include "qt/mubdi/forms/ui_masternodeswidget.h"
#include "qt/mubdi/qtutils.h"
#include "qt/mubdi/mnrow.h"
#include "qt/mubdi/mninfodialog.h"

#include "qt/mubdi/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(MuBdIGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN){
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the mubdi.conf file\nof your remote server and start it, "
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:46031 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * 5000)){
            inform(tr("Not enough balance to create a masternode, 5,000 CAI required."));
            return;
        }
        showHideOp(true);
