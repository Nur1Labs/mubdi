...
#include "qt/mubdi/forms/ui_sendconfirmdialog.h"
...
void TxDetailDialog::setData(WalletModel *model, const QModelIndex &index){
    ...
    QString amountText = MuBdIUnits::formatWithUnit(nDisplayUnit, amount, true, MuBdIUnits::separatorAlways);
...
void TxDetailDialog::setData(WalletModel *model, WalletModelTransaction &tx){
    ...
    ui->textAmount->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, totalAmount, false, MuBdIUnits::separatorAlways) + " (Fee included)");
    ...
    ui->textFee->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, txFee, false, MuBdIUnits::separatorAlways));
}
            ...
            if(tx) {
                for (const CTxOut &out : tx->vout) {
                    ...
                    if (ExtractDestination(out.scriptPubKey, dest, isCsAddress)) {
                        std::string address = ((isCsAddress) ? CMuBdIAddress::newCSInstance(dest) : CMuBdIAddress::newInstance(dest)).ToString();
                    ...
                    QLabel *label1 = new QLabel(MuBdIUnits::formatWithUnit(nDisplayUnit, out.nValue, false, MuBdIUnits::separatorAlways));
