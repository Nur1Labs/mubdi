...
#include "qt/mubdi/coldstakingwidget.h"
#include "qt/mubdi/forms/ui_coldstakingwidget.h"
#include "qt/mubdi/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/mubdi/requestdialog.h"
#include "qt/mubdi/tooltipmenu.h"
...
ColdStakingWidget::ColdStakingWidget(MuBdIGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your CAIs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept CAI delegation / Delegate CAI"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CAI outputs to delegate.");
...
void ColdStakingWidget::onSendClicked(){
    ...
    if (dest.amount < minColdStakingAmount) {
        inform(tr("Invalid entry, minimum delegable amount is ") +
               MuBdIUnits::formatWithUnit(nDisplayUnit, minColdStakingAmount));
        return;
    }
    ...
    GuiTransactionsUtils::ProcessSendCoinsReturnAndInform(
            ...
            prepareStatus,
            walletModel,
            MuBdIUnits::formatWithUnit(nDisplayUnit, currentTransaction.getTransactionFee()),
            true
    );
...
void ColdStakingWidget::onLabelClicked(QString dialogTitle, const QModelIndex &index, const bool& isMyColdStakingAddresses) {
    if(walletModel && !isShowingDialog) {
        ...
        if (openDialogWithOpaqueBackgroundY(dialog, window, 3.5, 6)) {
            ...
            const CMuBdIAddress address = CMuBdIAddress(stdString.data());
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 CAI" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
