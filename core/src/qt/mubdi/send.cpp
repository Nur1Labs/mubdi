...
#include "qt/mubdi/send.h"
#include "qt/mubdi/forms/ui_send.h"
#include "qt/mubdi/addnewcontactdialog.h"
#include "qt/mubdi/qtutils.h"
#include "qt/mubdi/sendchangeaddressdialog.h"
#include "qt/mubdi/optionbutton.h"
#include "qt/mubdi/sendconfirmdialog.h"
#include "qt/mubdi/myaddressrow.h"
#include "qt/mubdi/guitransactionsutils.h"
...
SendWidget::SendWidget(MuBdIGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("CAI");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("XAU");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (CAI) or backend proof (XAU)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("CArIa address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 CAI");
    ...
    setCssProperty(coinIcon, "coin-icon-mubdi");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onMuBdISelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onMuBdISelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZMuBdI = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZMuBdI));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = 0;
        ui->labelTitleTotalRemaining->setText(tr("Total remaining(zero deprecated)"));
    }
...
void SendWidget::onSendClicked(){
    ...
    bool sendMuBdI = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendMuBdI ? AskPassphraseDialog::Context::Send_MuBdI : AskPassphraseDialog::Context::Send_zMuBdI, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendMuBdI) ? send(recipients) : sendZMuBdI(recipients)) {
...
bool SendWidget::send(QList<SendCoinsRecipient> recipients){
    ...
    GuiTransactionsUtils::ProcessSendCoinsReturnAndInform(
            ...
            prepareStatus,
            walletModel,
            MuBdIUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(),
...
bool SendWidget::sendZMuBdI(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
}
...
QString SendWidget::recipientsToString(QList<SendCoinsRecipient> recipients){
    ...
    for (SendCoinsRecipient rec : recipients){
        s += rec.address + " -> " + MuBdIUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), rec.amount, false, MuBdIUnits::separatorAlways) + "\n";
    }
    return s;
}
    ...
    for (SendCoinsRecipient rec : recipients){
        ...
        if(!label.isNull()) {
            ...
            if(label.compare(labelOld) != 0) {
                CTxDestination dest = CMuBdIAddress(rec.address.toStdString()).Get();
...
void SendWidget::onChangeAddressClicked(){
    ...
    if(!boost::get<CNoDestination>(&CoinControlDialog::coinControl->destChange)){
        dialog->setAddress(QString::fromStdString(CMuBdIAddress(CoinControlDialog::coinControl->destChange).ToString()));
    }
    if(openDialogWithOpaqueBackgroundY(dialog, window, 3, 5)) {
        if(dialog->selected) {
            QString ret;
            if (dialog->getAddress(walletModel, &ret)) {
                CoinControlDialog::coinControl->destChange = CMuBdIAddress(ret.toStdString()).Get();
...
void SendWidget::onOpenUriClicked(){
    ...
    if (openDialogWithOpaqueBackgroundY(dlg, window, 3, 5)) {
        ...
        if (!GUIUtil::parseMuBdIURI(dlg->getURI(), &rcp)) {
        ...
        if (listSize == 1) {
            ...
            entry->setAmount(MuBdIUnits::format(nDisplayUnit, rcp.amount, false));
        } else {
...
void SendWidget::onChangeCustomFeeClicked(){
    ...
    if (openDialogWithOpaqueBackgroundY(customFeeDialog, window, 3, 5)){
        ui->pushButtonFee->setText(tr("Custom Fee %1").arg(MuBdIUnits::formatWithUnit(nDisplayUnit, customFeeDialog->getFeeRate().GetFeePerK()) + "/kB"));
...
void SendWidget::onCoinControlClicked(){
    if(isMuBdI){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CAI to select."));
        }
    }else{
            inform(tr("You don't have any XAU in your balance to select."));
    }
}
...
void SendWidget::onMuBdISelected(bool _isMuBdI){
    isMuBdI = _isMuBdI;
    setCssProperty(coinIcon, _isMuBdI ? "coin-icon-mubdi" : "coin-icon-zmubdi");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CMuBdIAddress MuBdIAdd = CMuBdIAddress(address.toStdString());
        if (walletModel->isMine(MuBdIAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(MuBdIAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
