...
#include "qt/mubdi/forms/ui_receivewidget.h"
...
#include "qt/mubdi/addressholder.h"
...
ReceiveWidget::ReceiveWidget(MuBdIGUI* parent) :
    PWidget(parent),
    ui(new Ui::ReceiveWidget)
{
    ...
    ui->labelSubtitle1->setText(tr("Scan the QR code or copy the address to receive CAI."));
...
void ReceiveWidget::refreshView(QString refreshAddress){
    try {
        ...
        int64_t time = walletModel->getKeyCreationTime(CMuBdIAddress(latestAddress.toStdString()));
...
void ReceiveWidget::updateQr(QString address){
    ...
    QString uri = GUIUtil::formatMuBdIURI(*info);
...
void ReceiveWidget::onLabelClicked(){
    if(walletModel && !isShowingDialog) {
        ...
        if (openDialogWithOpaqueBackgroundY(dialog, window, 3.5, 6)) {
            ...
            const CMuBdIAddress address = CMuBdIAddress(info->address.toUtf8().constData());
