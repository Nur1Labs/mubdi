...
#include "qt/mubdi/sendmultirow.h"
#include "qt/mubdi/forms/ui_sendmultirow.h"
...
bool SendMultiRow::addressChanged(const QString& str){
    if(!str.isEmpty()) {
        ...
        if (!valid) {
            ...
            if (GUIUtil::parseMuBdIURI(trimmedStr, &rcp)) {
                ui->lineEditAddress->setText(rcp.address);
                ui->lineEditAmount->setText(MuBdIUnits::format(displayUnit, rcp.amount, false));
