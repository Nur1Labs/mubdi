    ...
    void refreshAddressTable()
    {
        cachedAddressTable.clear();
        {
            ...
            for (const PAIRTYPE(CTxDestination, AddressBook::CAddressBookData) & item : wallet->mapAddressBook) {
                ...
                const CMuBdIAddress address = CMuBdIAddress(item.first, addrType);
    ...
    void updateEntry(const QString& address, const QString& label, bool isMine, const QString& purpose, int status)
    {
        ...
        case CT_NEW: {
            ...
            if (stdPurpose == AddressBook::AddressBookPurpose::RECEIVE)
                creationTime = static_cast<uint>(wallet->GetKeyCreationTime(CMuBdIAddress(address.toStdString())));
...
bool AddressTableModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        CTxDestination curAddress = CMuBdIAddress(rec->address.toStdString()).Get();
        if (index.column() == Label) {
            ...
            CTxDestination newAddress = CMuBdIAddress(value.toString().toStdString()).Get();
...
void AddressTableModel::updateEntry(const QString& address,
    ...
    int status)
{
    // Update address book model from MuBdI core
...
{
    // Update stealth address book model from MuBdI core
...
QString AddressTableModel::addRow(const QString& type, const QString& label, const QString& address)
{
    ...
    if (type == Send) {
        ...
        {
            LOCK(wallet->cs_wallet);
            if (wallet->mapAddressBook.count(CMuBdIAddress(strAddress).Get())) {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
        }
        ...
        strAddress = CMuBdIAddress(newKey.GetID()).ToString();
    } else {
        return QString();
    }
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBook(CMuBdIAddress(strAddress).Get(), strLabel,
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex& parent)
{
    ...
    {
        LOCK(wallet->cs_wallet);
        return wallet->DelAddressBook(CMuBdIAddress(rec->address.toStdString()).Get(), addrType);
    }
}
...
QString AddressTableModel::labelForAddress(const QString& address) const
{
    ...
    if (!address.isEmpty()) {
        {
            LOCK(wallet->cs_wallet);
            CMuBdIAddress address_parsed(address.toStdString());
...
{
    return wallet->purposeForAddress(CMuBdIAddress(address).Get());
}
    ...
    if(!wallet->mapAddressBook.empty()) {
        for (auto it = wallet->mapAddressBook.rbegin(); it != wallet->mapAddressBook.rend(); ++it ) {
            if (it->second.purpose == AddressBook::AddressBookPurpose::RECEIVE) {
                const CMuBdIAddress &address = it->first;
