...
#ifndef MUBDI_QT_WALLETMODEL_H
#define MUBDI_QT_WALLETMODEL_H
...
/** Interface to MuBdI wallet from Qt view code. */
...
public:
    ...
    PairResult getNewAddress(CMuBdIAddress& ret, std::string label = "") const;
    /**
     * Return a new address used to receive for delegated cold stake purpose.
     */
    PairResult getNewStakingAddress(CMuBdIAddress& ret, std::string label = "") const;
    ...
    std::string getLabelForAddress(const CMuBdIAddress& address);
    bool getKeyId(const CMuBdIAddress& address, CKeyID& keyID);

    bool isMine(CMuBdIAddress address);
    bool isMine(const QString& addressStr);
    bool isUsed(CMuBdIAddress address);
