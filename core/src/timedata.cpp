...
#define MUBDI_TIMEDATA_MAX_SAMPLES 200
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample, int nOffsetLimit)
{
    ...
    if (setKnown.size() == MUBDI_TIMEDATA_MAX_SAMPLES)
        return;
    if (!Params().IsRegTestNet() && !setKnown.insert(ip).second)
        return;

    // Add data
    static CMedianFilter<int64_t> vTimeOffsets(MUBDI_TIMEDATA_MAX_SAMPLES, 0);
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        ...
        if (abs64(nMedian) < nOffsetLimit) {
            ...
            std::string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MuBdI Core will not work properly.");
