...
BOOST_AUTO_TEST_CASE(budget_value)
{
    ...
    int nHeightTest = Params().Zerocoin_StartTime() + 1;
    CheckBudgetValue(nHeightTest, "testnet", 4500*COIN);

    SelectParams(CBaseChainParams::MAIN);
    nHeightTest = Params().Zerocoin_StartTime() + 1;
    CheckBudgetValue(nHeightTest, "mainnet", 45200*COIN);
}
