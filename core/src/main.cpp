...
/** Fees smaller than this (in umubdi) are considered zero fee (for relaying and mining)
 * We are ~100 times smaller then mubdi now (2015-06-23), set minRelayTxFee only 10 times higher
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (!CheckTransaction(tx, chainHeight >= Params().Zerocoin_StartHeight(), true, state, false, fColdStakingActive))
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : XAU spend tx %s already in block %d",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    const int start_pos_block = Params().GetConsensus().height_start_PoS;
    ...
    if (Params().NetworkID() == CBaseChainParams::LIESE) {
        if (nHeight < start_pos_block && nHeight > 0)
            return 100 * COIN;
        else
            return 5 * COIN;
    }

    if (Params().NetworkID() == CBaseChainParams::ARIN) {
        if (nHeight < start_pos_block && nHeight > 0)
            return 1000 * COIN;
        else
            return 50 * COIN;
    ...
    if (Params().NetworkID() == CBaseChainParams::GAZA) {
        if (nHeight < start_pos_block && nHeight > 0)
            return 500 * COIN;
        else
            return 5 * COIN;
    ...
    if (nHeight == 0) {
        nSubsidy = 5908800 * COIN;
    } else if (nHeight < 57720 && nHeight > 0) {
        nSubsidy = 100 * COIN;
    } else if (nHeight < (Params().NetworkID() == CBaseChainParams::TESTNET ? 121140 : 127340) && nHeight >= 57720) {			            //Beta.P2 Chain healthly
        nSubsidy = 80 * COIN;
    } else if (nHeight <= last_pow_block && nHeight >= 127340) {	//last_pow=196960
        nSubsidy = 47.5 * COIN;
    } else if (nHeight <= 254679 && nHeight > last_pow_block) {
        nSubsidy = 47.5 * COIN;
    } else if (nHeight <= 312399 && nHeight >= 254680) {
        nSubsidy = 40.5 * COIN;
    } else if (nHeight <= 370119 && nHeight >= 312400) {
        nSubsidy = 38 * COIN;
    } else if (nHeight <= 427839 && nHeight >= 370120) {
        nSubsidy = 31.5 * COIN;
    } else if (nHeight <= 485559 && nHeight >= 427840) {
        nSubsidy = 27 * COIN;
    } else if (nHeight <= 543279 && nHeight >= 485560) {
        nSubsidy = 18 * COIN;
    } else if (nHeight <= 600999 && nHeight >= 543280) {
        nSubsidy = 9 * COIN;
    } else if (nHeight >= 601000) {                       //>= 601000 infinity 
        nSubsidy = 4.5 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    if (mNodeCoins == 0) {
        ret = 0;
    } else if (nHeight <= 300500) {        //601000/2
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight <= 43200) {
        ret = blockValue / 5;							//20% for Masternode
    } else if (nHeight < 57720 && nHeight > 43200) {
        ret = blockValue / (100 / 48);						//17% for Masternode 1/3 & 2/3
    } else if (nHeight < (Params().NetworkID() == CBaseChainParams::TESTNET ? 121140 : 127340) && nHeight >= 57720) {
        ret = 50 * COIN;							//fixed MN reward equal to 42% for specific phase
    } else if (nHeight <= consensus.height_last_PoW && nHeight >= 127340) {	//127340-POW_END
        ret = blockValue / 100 * 70;						//70% for Masternode
    } else if (nHeight < Params().Zerocoin_StartTime()) {
        return GetSeeSaw(blockValue, nMasternodeCount, nHeight);
    } else {
        ret = blockValue / 100 * 60;						//72% if seesaw not active after PoW end
    }
...
bool RecalculateMuBdISupply(int nHeightStart)
{
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (!Params().IsRegTestNet() && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 100/*150000*/)) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> MuBdIInputs;
        std::vector<CTxIn> zMuBdIInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zMuBdIInputs.push_back(stakeIn);
            }else{
                MuBdIInputs.push_back(stakeIn);
            }
        }
        const bool hasMuBdIInputs = !MuBdIInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasMuBdIInputs)
                    ...
                    for (const CTxIn& MuBdIIn : MuBdIInputs){
                        if(MuBdIIn.prevout == in.prevout){
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : MuBdIInputs) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MuBdI: We use certain sporks during IBD, so check to see if they are
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in MuBdI
