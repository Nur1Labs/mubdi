...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "mubdi" is a composite category enabling all mubdi-related debug output
            if (ptrCategory->count(std::string("mubdi"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MuBdI
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MuBdI
// Mac: ~/Library/Application Support/MuBdI
// Unix: ~/.mubdi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CArIa";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "caria.conf"));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "mubdi-mn.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mubdi.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mubdi.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cariad.pid"));
