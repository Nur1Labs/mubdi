...
echo -e "Preparing the VPS to setup. ${CYAN}$COIN_NAME${NC} ${RED}Exchanger Nodes${NC}"
...
echo -e "Preparing the VPS to setup. ${CYAN}$COIN_NAME${NC} ${RED}Exchanger_Web Component${NC}"
#this for autoinstall
sudo apt-get update >/dev/null 2>&1
echo -e "Installing web packages, it may take some time to finish.${NC}"
sudo apt install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" apache2 php libapache2-mod-php php-mysql php-cli curl >/dev/null 2>&1
sudo ufw allow in "Apache Full"
if [ "$?" -gt "0" ];
  then
    echo -e "${RED}Not all required packages were installed properly. Try to install them manually by running the following commands:${NC}\n"
    echo "sudo apt update"
    echo "sudo apt -y apache2 php libapache2-mod-php php-mysql php-cli curl"
 exit 1
fi
}
...
function important_information() {
 ...
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "$COIN_NAME Exchanger Nodes is up and running listening on port ${GREEN}$COIN_PORT${NC}."
