...
if [[ $(lsb_release -d) != *10* ]]; then
  echo -e "${RED}You are not running Debian 10.x. Installation is cancelled.${NC}"
  exit 1
fi
...
apt install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" git unzip curl >/dev/null 2>&1
...
if [ "$?" -gt "0" ];
  then
    ...
    echo "apt update"
    echo "apt install -y git unzip curl &&
...
#main
cd ~/coins/caistream/core && ./mubdi.sh && ./configure --enable-cxx --enable-shared --enable-hardening --disable-tests --without-gui --with-incompatible-bdb --with-unsupported-ssl && make && make install
}
